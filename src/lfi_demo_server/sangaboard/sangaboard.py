...
class Sangaboard(ExtensibleSerialInstrument):
    ...
    # Since Sangaboard v4, we need to also try using a lower baud rate when we autodetect.
    port_settings_list = [
        {
            "baudrate": 115_200,
            "bytesize": EIGHTBITS,
    ...
    def __init__(self, port=None, timeout: int = 2, **kwargs):
        ...
        Note that the list of port settings is only used when autodetecting a port.  If you
        want to use an alternative baud rate, for example, it can be passed as an additional
        keyword argument.
        """
        ...
        try:
            ...
            if self.version_tuple[0] == 1:
                ...
                logging.info("Enabling backwards-compatible behaviour on new Sangaboard firmware")
    ...
    def check_valid_firmware(self):
        ...
        # Check for valid firmware string. Version group is matched later with semantic_version
        match = re.match(
            r"(Sangaboard Firmware|OpenFlexure Motor Board) v(.*)\s*$",
            self.firmware
        )
        if not match:
            logging.warning(
                f'Version string "{self.firmware}" was not recognised as a supported version.'
            )
            return False
        ...
        try:
            ...
            logging.warning(
                f'Could not parse version string "{self.firmware_version}" as a semantic version.'
            )
            return False
        self.version_tuple = (parsed_version.major, parsed_version.minor)
        if parsed_version.prerelease:
            logging.warning(
                f'You are using a Sangaboard with prerelease firmware "{self.firmware_version}"'
            )
